...
export const makeGroupsSocket = (config) => {
    ...
    const { authState, ev, query, upsertMessage, disposables, signalRepository } = sock;
    ...
    const groupMetadata = async (jid) => {
        ...
        return extractGroupMetadata(result, sock);
    };
    ...
    sock.ws.on('CB:ib,,dirty', onDirty);
    disposables.push(() => sock.ws.off('CB:ib,,dirty', onDirty));
    return {
        ...sock,
        groupMetadata,
        groupCreate: async (subject, participants) => {
            const key = generateMessageIDV2();
            const content = await Promise.all(participants.map(async (jid) => {
                let ptContent = undefined;
                const tcNodeArray = await buildTcTokenFromJid({
                    jid,
                    authState,
                    getLIDForPN: signalRepository.lidMapping.getLIDForPN.bind(signalRepository.lidMapping)
                });
                if (tcNodeArray && tcNodeArray.length) {
                    ptContent = [{
                            tag: 'privacy',
                            attrs: {},
                            content: tcNodeArray[0]?.content
                        }];
                }
                return {
                    tag: 'participant',
        ...
        groupParticipantsUpdate: async (jid, participants, action) => {
            const content = await Promise.all(participants.map(async (pJid) => {
                let ptContent = undefined;
                if (action === 'add') {
                    const tcNodeArray = await buildTcTokenFromJid({
                        jid: pJid,
        ...
        groupAcceptInvite: async (code) => {
            code = code.replace(/.*whatsapp\.com.*\//, '').replace(/\?.*/, '');
        ...
        groupGetInviteInfo: async (code) => {
            ...
            return extractGroupMetadata(results, sock);
        },
...
export const extractGroupMetadata = (result, sock) => {
    ...
    const metadata = {
        ...
        participants: getBinaryNodeChildren(group, 'participant').map(({ attrs }) => {
            ...
            if (isLidUser(jid) && isPnUser(attrs.phone_number)) {
                participant.phoneNumber = attrs.phone_number;
            }
            else if (isPnUser(jid) && isLidUser(attrs.lid)) {
                participant.lid = attrs.lid;
            }
            if (participant.lid && participant.id && sock) {
                sock.signalRepository.lidMapping.storeLIDPNMappings([{ lid: participant.lid, pn: participant.id }]).catch(err => {
                    sock.logger.warn({ err, lid: participant.lid, pn: participant.id }, 'failed to store lid mapping from group metadata');
                });
            }
            return participant;
        }),
