...
import { getBinaryNodeChild, getBinaryNodeChildren, isJidGroup, isLidUser, isPnUser, jidNormalizedUser } from '../WABinary/index.js';
import { aesDecrypt, aesEncrypt, constantTimeEqual, hkdf, hmacSign } from './crypto.js';
...
export const decodeSyncdMutations = async (msgMutations, initialState, getAppStateSyncKey, onMutation, validateMacs) => {
    ...
    for (const msgMutation of msgMutations) {
        ...
        if (validateMacs) {
            ...
            if (!constantTimeEqual(contentHmac, ogValueMac)) {
        ...
        if (validateMacs) {
            ...
            if (!constantTimeEqual(hmac, record.index.blob)) {
...
export const decodeSyncdPatch = async (msg, name, initialState, getAppStateSyncKey, onMutation, validateMacs) => {
    if (validateMacs) {
        ...
        if (!constantTimeEqual(patchMac, msg.patchMac)) {
...
export const decodeSyncdSnapshot = async (name, snapshot, getAppStateSyncKey, minimumVersionNumber, validateMacs = true, logger) => {
    ...
    if (validateMacs) {
        ...
        if (!constantTimeEqual(snapshot.mac, computedSnapshotMac)) {
...
export const decodePatches = async (name, syncds, initial, getAppStateSyncKey, options, minimumVersionNumber, logger, validateMacs = true) => {
    ...
    for (const syncd of syncds) {
        ...
        if (validateMacs) {
            ...
            const result = mutationKeys(keyEnc.keyData);
            const computedSnapshotMac = generateSnapshotMac(newState.hash, newState.version, name, result.snapshotMacKey);
            if (!constantTimeEqual(snapshotMac, computedSnapshotMac)) {
...
export const processSyncAction = (syncAction, ev, me, initialSyncOpts, logger) => {
    ...
    const { syncAction: { value: action }, index: [type, id, msgId, fromMe], operation } = syncAction;
    if (action?.muteAction) {
        ...
        // NCT salt for cstoken derivation — account-level, persisted in creds.
        // REMOVE clears it; SET stores the new salt (rotations arrive as SET).
        const salt = operation === proto.SyncdMutation.SyncdOperation.REMOVE
            ? undefined
            : (action.nctSaltSyncAction.salt ?? undefined);
