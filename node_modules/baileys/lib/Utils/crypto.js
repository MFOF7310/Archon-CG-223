 ...
 */
export function toMessageSecretBytes(secret) {
    if (!secret) {
        return undefined;
    }
    ...
    if (typeof secret === 'object' && 'data' in secret) {
        const data = secret.data;
        return Buffer.isBuffer(data) || Array.isArray(data) ? Buffer.from(data) : Buffer.from(String(data), 'base64');
    }
    return undefined;
}
export const Curve = {
...
export function aesDecryptWithIV(buffer, key, IV) {
    if (buffer.length === 0 || buffer.length % 16 !== 0) {
        throw new Error(`ciphertext is not a multiple of block size: ${buffer.length} / 16`);
    }
