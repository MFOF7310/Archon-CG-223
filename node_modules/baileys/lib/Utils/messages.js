...
import { generateMessageIDV2, generateParticipantHashV2, getKeyAuthor, unixTimestampSeconds } from './generics.js';
import { downloadContentFromMessage, encryptedStream, generateThumbnail, getAudioDuration, getAudioWaveform, getRawMediaUploadData, getStatusCodeForMediaRetry, getWAUploadToServer } from './messages-media.js';
...
export const generateWAMessageFromContent = (jid, message, options) => {
    ...
    if (quoted && !isJidNewsletter(jid)) {
        const participant = jidNormalizedUser(quoted.key.fromMe
            ? userJid
            : (quoted.participant || quoted.key.participant || (quoted.key.remoteJid && (isPnUser(quoted.key.remoteJid) || isLidUser(quoted.key.remoteJid))
...
export const getMediaType = (message) => {
    ...
    else if (message.extendedTextMessage?.title) {
        return 'url';
    }
    else if (message.reactionMessage || message.encReactionMessage) {
        return 'reaction';
    }
