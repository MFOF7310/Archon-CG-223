...
export const checkCertValidity = (details, label) => {
    const nowSeconds = Math.floor(Date.now() / 1000);
    const notBefore = details.notBefore ? Number(details.notBefore) : 0;
    const notAfter = details.notAfter ? Number(details.notAfter) : 0;
    if (notBefore && nowSeconds < notBefore) {
        throw new Boom(`${label} cert not valid yet (now ${nowSeconds} < notBefore ${notBefore})`, { statusCode: 400 });
    }
    if (notAfter && nowSeconds > notAfter) {
        throw new Boom(`${label} cert expired (now ${nowSeconds} > notAfter ${notAfter})`, { statusCode: 400 });
    }
};
...
export const makeNoiseHandler = ({ keyPair: { private: privateKey, public: publicKey }, NOISE_HEADER, logger, routingInfo }) => {
    ...
    return {
        ...
        processHandshake: ({ serverHello }, noiseKey) => {
            ...
            checkCertValidity(details, 'intermediate');
            checkCertValidity(leafDetails, 'leaf');
        ...
        decodeFrame: async (newData, onFrame) => {
            return decodeMutex.mutex(async () => {
